    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				float closeness = 0;
				float coloredness = 0;
				float theNoise = snoise(i.posWorld * 3);
				for (int j = 0; j < 25; j++) {
					float4 thePoint = VisionPoints[j];
					float dist = distance(i.posWorld, thePoint);
					dist += (theNoise-1);
                ...
                float3 _multiplier7 = (float3((_WaveSpeed/_division1),0.0)*(_timer2.r/100.0));
                float2 _multiplier5 = ((_rotator1+_multiplier7)*_division1);
                float4 _texture1 = tex2D(_NormalTexture,TRANSFORM_TEX(_multiplier5, _NormalTexture));
...
/////// Diffuse:
                ...
                float _value1 = 1.0;
                float3 _blend1 = saturate((_DeepWaterColor.rgb+(sceneColor.rgb*pow(saturate((_value1 + ( ((sceneZ-partZ) - _vector31) * (0.5 - _value1) ) / (((_MainColor.rgb*(10.0/_Density))+0.0) - _vector31))),_Fade))));
                float2 _mask1 = _subtractor1.rg;
                float2 _remap1 = ((i.screenPos.rg+(float2(_mask1.r,_mask1.g)*_Distortion))*0.5+0.5);
                float4 _ReflectionTex_var = tex2D(_ReflectionTex,TRANSFORM_TEX(_remap1, _ReflectionTex));
                ...
                float3 _multiplier8 = (float3((_FoamSpeed/_division2),0.0)*(_timer1.r/100.0));
                float2 _multiplier4 = ((_rotator2+_multiplier8)*_division2);
                float4 _texture3 = tex2D(_FoamTexture,TRANSFORM_TEX(_multiplier4, _FoamTexture));
                float2 _multiplier3 = ((i.uv0+_multiplier8)*_division2);
                float4 _texture4 = tex2D(_FoamTexture,TRANSFORM_TEX(_multiplier3, _FoamTexture));
                float _value2 = 0.0;
